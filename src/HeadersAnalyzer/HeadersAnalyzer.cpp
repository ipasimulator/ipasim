...
int main() {
  ...
  // lower.
  {
    ...
    for (const auto &Lib : iOSLibs) {
      fstream OutputFile(
          (OutputDir / to_string(I)).replace_extension(".mm").c_str(),
          fstream::out);

      // Generate header.
      OutputFile << "// Wrappers for iOS library `" << Lib.Name << "`.\n"
                 << "// This file was generated by `HeadersAnalyzer`.\n\n"
                 << "#include \"iOSHeaders.mm\"\n\n";

      // TODO: Generate function wrappers.
      for (const auto *Exp : Lib.Exports) {
        OutputFile << "// TODO: Generate wrapper for function `" << Exp->Name
                   << "`.\n";
      }
