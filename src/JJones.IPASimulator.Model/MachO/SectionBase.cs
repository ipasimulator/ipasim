namespace JJones.IPASimulator.Model.MachO
{
    public abstract class SectionBase
    {
        ...
        public SectionBase(string sectionName, string segmentName, uint offset, uint align, uint relOffset, uint nReloc, uint flags, uint reserved1, uint reserved2)
            : this(sectionName, segmentName, offset, align, relOffset, nReloc,
                  (SectionType)(flags & (uint)SectionFlagsMask.Type),
                  (SectionAttributes)(flags & (uint)SectionFlagsMask.Attributes),
                  reserved1, reserved2)
        {
        }
        public SectionBase(string sectionName, string segmentName, uint offset, uint align, uint relOffset, uint nReloc, SectionType type, SectionAttributes attributes, uint reserved1, uint reserved2)
        {
